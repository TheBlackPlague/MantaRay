...
namespace MantaRay
{
    ...
    class SIMD
    {
        ...
        static inline void AddToAll(      std::array<T, InputSize>& inputA,
        ...
        static void ActivateFlattenAndForward(
            const std::array<T , InputSize                 >& inputA,
            ...
            const uint32_t offset)
        {
            ...
            for (size_t i = 0; i < OutputSize; i++) {
                ...
                output[offset + i] = Avx512<OT>::Sum(zmm0) + bias[offset + i];
#elifdef __AVX2__
                // Define the register for sum accumulation:
                ...
                output[offset + i] = Avx2<OT>::Sum(ymm0) + bias[offset + i];
#else
