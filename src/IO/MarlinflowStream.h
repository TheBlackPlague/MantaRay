...
namespace MantaRay
{
    ...
    /// \details This class is used to read Marlinflow Neural Network JSON data.
    class MarlinflowStream final : DataStream<std::ios::in>
    {
        ...
        explicit MarlinflowStream(const std::string& path) : DataStream(path)
        {
        ...
        void Read2DArray(const std::string& key, std::array<T, Size>& array,
                         const size_t stride,
                         const size_t K,
                         const bool permute)
        {
            ...
            for (auto& [k, v] : obj.items()) {
                ...
                for (auto& [k2, v2] : v.items()) {
                    ...
                    const auto d = static_cast<double>(v2);
